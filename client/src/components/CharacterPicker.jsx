...
import { SSBU_CHARACTERS } from '../data/ssbu-characters';
...
export default function CharacterPicker({ matchId, currentCharacter, player, onUpdate }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="text-lg">{currentChar ? '🎮' : '👤'}</span>
        <span className="flex-1 text-left truncate">{currentChar?.name || 'Seleccionar personaje'}</span>
        <span className="text-xs text-gray-500">{open ? '▲' : '▼'}</span>
      </button>

      {open && (
        <div className="absolute z-50 mt-1 w-full bg-dark-light border border-gray-600 rounded-lg shadow-xl max-h-64 overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto flex-1">
            {filtered.map(c => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-sm hover:bg-primary/20 transition flex items-center gap-2 ${
                  currentCharacter === c.id ? 'bg-primary/10 text-primary' : 'text-gray-300'
                }`}
              >
                <span className="text-base">🎮</span>
                <span>{c.name}</span>
                {currentCharacter === c.id && <span className="ml-auto text-green-400">✓</span>}
              </button>
            ))}
            {filtered.length === 0 && (
