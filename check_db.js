...
console.log(JSON.stringify(db.prepare('SELECT id, name, status, current_match_order FROM tournaments').all(), null, 2));
console.log('=== PARTICIPANTS ===');
console.log(JSON.stringify(db.prepare('SELECT id, tournament_id, user_id, name FROM participants').all(), null, 2));
console.log('=== MATCHES (first 5 by order) ===');
console.log(JSON.stringify(db.prepare('SELECT id, tournament_id, bracket_type, match_order, round, player1_id, player2_id, status FROM matches ORDER BY match_order LIMIT 10').all(), null, 2));
console.log('=== USERS ===');
