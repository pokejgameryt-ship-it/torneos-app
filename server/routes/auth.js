...
router.post('/register', (req, res) => {
  ...
  if (!email || !nickname || !password) return res.status(400).json({ error: 'Email, nickname y contraseña requeridos' });
  if (password.length < 4) return res.status(400).json({ error: 'La contraseña debe tener al menos 4 caracteres' });
  const db = getDb();
  const existing = db.prepare('SELECT id FROM users WHERE email = ? OR nickname = ?').get(email, nickname);
  if (existing) return res.status(400).json({ error: 'Email o nickname ya existen' });
  const id = uuidv4();
  ...
  const user = db.prepare('SELECT id, email, nickname, display_name, bio, games, avatar, country, continent, default_nickname, default_flag FROM users WHERE id = ?').get(id);
  res.json({ token, user });
});

router.post('/login', (req, res) => {
  const { login, password } = req.body;
  if (!login || !password) return res.status(400).json({ error: 'Email/nickname y contraseña requeridos' });
  const db = getDb();
  ...
  res.json({ token, user: { id: user.id, email: user.email, nickname: user.nickname, display_name: user.display_name, bio: user.bio, games: user.games, avatar: user.avatar, country: user.country, continent: user.continent, default_nickname: user.default_nickname, default_flag: user.default_flag } });
});
